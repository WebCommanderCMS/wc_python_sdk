...
from wc_py.http.http_const import HTTPConst
...
class HTTPRequester:
    ...
    def delete(self, url: str, json_dict: dict = None, params: dict = None) -> HTTPResponse:
        url = self._get_url(url)
        response = requests.delete(url, headers=self.headers, json=json_dict, params=params)
        return HTTPResponse.get_response(response)
    ...
    def add_bearer_token(self, token) -> 'HTTPRequester':
        self.add_header("access_token", f"{token}")
        return self
